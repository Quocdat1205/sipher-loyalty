...
export class LootboxTrackerService {
  ...
  private trackingTranserBatch = async (_fromBlock: number) => {
    ...
    const pastEvents = await this.contract.queryFilter(filter, _fromBlock, _currentBlock).catch(err => {
    ...
    pastEvents.forEach(event => {
      ...
      if (from !== ZERO_ADDRESS) {
        const index = tokenCheckers.findIndex(token => token.publicAddress === from)
        if (index === -1) tokenCheckers.push({ publicAddress: from, ids })
        else tokenCheckers[index].ids.concat(ids)
      }
      if (to !== ZERO_ADDRESS) {
        const index = tokenCheckers.findIndex(token => token.publicAddress === to)
  ...
  private trackingMintedBatch = async (_fromBlock: number, _currentBlock: number) => {
    ...
    pastEvents.forEach(event => {
      ...
      promises.push(this.lootBoxService.updateLootboxFromTrackerBatchOrder(batchOrder))
    })
    await Promise.all(promises)
    return _currentBlock + 1
  }

  private trackingMintedSingle = async (_fromBlock: number, _currentBlock: number) => {
