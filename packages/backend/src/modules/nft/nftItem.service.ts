...
export class NftItemService {
  ...
  async search(options?: NftItemFilterDto) {
    const { owner, collections, listingTypes, rarities, priceFrom, priceTo, attributes, orderBy } = options
    const take = 20
    const from = 0
  ...
  private buildAttributeSearchOptions = (attributes: NftItemAttributeFilterDto[]) => {
