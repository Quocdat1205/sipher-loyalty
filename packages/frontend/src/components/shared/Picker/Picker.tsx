...
import PickerMixin, { IPickerProps } from './PickerMixin';
...
class Picker extends React.Component<IPickerProp & IPickerProps, any> {
  ...
  componentDidMount() {
    const { contentRef, indicatorRef, rootRef } = this;
  ...
  render() {
    const { props } = this;
    const { itemStyle, indicatorStyle, children } = props;
    const { selectedValue } = this.state;
    const map = (item: any) => {
      const { style, value } = item.props;
      return (
        <Box
          lineHeight="34px"
          whiteSpace="nowrap"
          ...
          h="34px"
          px={2}
          color={selectedValue === value ? 'white' : 'neutral.400'}
          key={value}
          {...itemStyle}
          {...style}
        >
    ...
    return (
      ...
      >
        <Box
          ...
          {...indicatorStyle}
        />
        <Box pos="absolute" left="0" top="0" w="100%" ref={el => (this.contentRef = el)}>
          {items}
        </Box>
      </Box>
    );
